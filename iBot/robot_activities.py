...
class Robot:
    def __init__(self, robotId, ExecutionId, url, ip, port, username, password, params):
        ...
        self.params = params
        response = requests.post(f"https://{self.url}/api-token-auth/", {'username': self.username, 'password': self.password})
    ...
    def findQueuesByName(self, queueName):
        Queues = []
        queues = requests.get(f'https://{self.url}/api/queues/QueueName={queueName}/',
    ...
    async def sendExecution(self, message, type='log'):
        ...
        uri = f"wss://{self.url}/ws/execution/{self.ExecutionId}/"
...
class Queue:
    def __init__(self, robotId, url, token, queueName=None, queueId=None):
        ...
        if queueId is None:
            ...
            requests.post(f'https://{self.url}/api/queues/',
                          {'RobotId': self.robotId, 'QueueId': self.queueId, 'QueueName': self.queueName},
                          headers={'Authorization': f'Token {self.token}'})
        else:
            self.queueId = queueId
            response = requests.get(f'https://{self.url}/api/queues/QueueId={self.queueId}/',
                                    headers={'Authorization': f'Token {self.token}'})
            self.queueName = response.json()['QueueName']

    def __getItem(self):
        queueItems = requests.get(f'https://{self.url}/api/items/QueueId={self.queueId}',
                                  headers={'Authorization': f'Token {self.token}'}).json()
...
class Item(Queue):
    def __init__(self, queueId, url, token, itemId=None, value=None):
        ...
        if itemId is None:
            ...
            if type(value) is dict:
                ...
                requests.post(f'https://{self.url}/api/items/', itemData,
                              headers={'Authorization': f'Token {self.token}'})
            else:
                raise ValueError("Item data must be a dictionary")
        else:
            self.itemId = itemId
            item = requests.get(f'https://{self.url}/api/items/ItemId={self.itemId}',
                                headers={'Authorization': f'Token {self.token}'}).json()
    ...
    def setItemAsWorking(self):
        ...
        requests.put(f'https://{self.url}/api/items/{self.itemId}/', data,
                     headers={'Authorization': f'Token {self.token}'})
